//
//  APIDataCodableArchiverClient.swift
...
// this will save data coming from the Google and NYTimes API calls

class APIDataCodableArchiverClient{
    //using a singleton to manage the model
    private init(){}
    static let manager = APIDataCodableArchiverClient()
    ...
    func dataFilePath(withPathName path: String) -> URL {
        ...
        return APIDataCodableArchiverClient.manager.documentsDirectory().appendingPathComponent(path)
    }
    ...
    private func saveCategories() -> Bool {
        ...
        do{
            ...
            try data.write(to: dataFilePath(withPathName: APIDataCodableArchiverClient.kPathName), options: .atomic)
            return true
        }catch{
    ...
    func loadCategories(){//decode the data into foundation object (cards)
        ...
        let path = dataFilePath(withPathName: APIDataCodableArchiverClient.kPathName)
